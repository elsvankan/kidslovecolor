...
| STRIPE_ENABLE_WECHAT_PAY | false | false totdat Stripe dit specifieke model schriftelijk heeft bevestigd |
...
De koppeling start expliciet met kaarten, waaronder in aanmerking komende
China UnionPay-kaarten van bezoekers. Apple Pay en Google Pay verschijnen
...
fysiek product? Vraag afzonderlijk naar WeChat Pay en Alipay en noem expliciet
Alipay's verbod op donations. Activeer WeChat Pay alleen als de bevestiging ook
dit model en dit account dekt: zet hem dan aan in het live Stripe-dashboard,
test in de sandbox, en zet pas daarna STRIPE_ENABLE_WECHAT_PAY=true voor de
bedoelde Vercel-omgeving. De code biedt dan card en wechat_pay aan. De
standaardwaarde is false; Alipay is bewust niet in de code opgenomen. Een
andere naam geven aan dezelfde bijdrage, zoals koffie of een ijsje, verandert
de aard van de betaling niet.

Op 20 september 2026 waren Alipay en WeChat Pay in de live Stripe-configuratie
zichtbaar maar uitgeschakeld. Dit is geen toelating. De live website gebruikt
nog Mollie en er zijn nog geen Stripe-sleutels in Vercel ingesteld. Dat is een
momentopname; controleer deze status voor activering opnieuw.

## Meten van belangstelling

De homepageknoppen sturen vanaf deze versie gescheiden GA4-events:
`support_coffee_click` en `support_icecream_click` bij een klik,
`support_coffee_checkout_open` en `support_icecream_checkout_open` zodra een
veilige betaallink is aangemaakt, en overeenkomstige `_error`-events bij een
technische fout. Ze bevatten alleen bedrag, valuta, taal en bij het openen de
provider; geen persoons- of betaalgegevens. In GA4 kun je per event het land
China kiezen. Oudere klikken zijn hiermee niet achteraf te reconstrueren.
Een checkout-open-event is geen geslaagde betaling; betaalstatus staat bij de
provider. Het event kan bij snelle navigatie door browserbeperkingen ontbreken.
...
- Laat STRIPE_ENABLE_WECHAT_PAY=false tot de methode expliciet is toegestaan en getest.
