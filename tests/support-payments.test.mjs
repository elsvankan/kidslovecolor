...
import { SUPPORT_COPY, validCheckoutUrl, supportOption, stripeSecret, mollieSecret } from "../lib/support-payments.mjs";
...
const envKeys = ["SUPPORT_PAYMENT_PROVIDER", "STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET", "MOLLIE_API_KEY", "VERCEL_ENV"];
...
function client({ lang = "nl", search = "", returnPage = false, providerResult = { status: "paid" }, ok = true, storage = new Map() } = {}) {
  const elements = Object.fromEntries(["paymentStatus", "paymentTitle", "paymentRetry", "paymentBack", "supportNote"].map(id => [id, fakeElement()]));
  const button = fakeElement(); button.dataset.support = "coffee";
  ...
  return { elements, button, context, replaced: () => replaced, assigned: () => assigned, submitted: () => submitted, requested: () => requested };
}
...
test("Buttons pass language and idempotency; API errors and unsafe destinations re-enable them", async () => {
  const c = client({ lang: "en", providerResult: { checkoutUrl: "https://checkout.stripe.com/c/pay/test" } });
