(function () {
  ...
  // Separate event names make coffee and ice-cream clicks countable in GA4
  // without a custom dimension. No identity or payment details are sent.
  function trackSupport(stage, support, lang, provider) {
    if (typeof window.gtag !== 'function') return;
    var item = support === 'bubble' ? 'icecream' : 'coffee';
    var details = { currency: 'EUR', value: support === 'bubble' ? 5 : 3, support_language: lang };
    if (provider === 'stripe' || provider === 'mollie') details.payment_provider = provider;
    window.gtag('event', 'support_' + item + '_' + stage, details);
  }
  ...
  function init() {
    ...
    buttons.forEach(function (button) {
      button.addEventListener('click', async function () {
        ...
        trackSupport('click', support, lang);
        setBusy(true, button);
        say(text.opening);
        try {
          ...
          trackSupport('checkout_open', support, lang, result.provider);
          window.location.assign(result.checkoutUrl);
        } catch (_) {
          trackSupport('error', support, lang);
          setBusy(false);
          say(text.error);
        }
      });
